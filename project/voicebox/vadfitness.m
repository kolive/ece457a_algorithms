...
%create time chunks for each set of tags
...
for j=1:analysisrange,
   ...
   if((fecmode == 1 || mscmode == 1) && rtags(i) == vtags(j) || rtags(i) == 0)
      ...
      %if rtags is not zero, then we're in msc mode, otherwise we are in
      %neither mode
      if(rtags(i) == 1)
          mscmode = 1;
      else
...
for j=1:analysisrange,
   ...
   if((overmode == 1 || ndsmode == 1) && rtags(i) == vtags(j) || rtags(i) == 1)
      ...
      %if rtags is zero, then we're in NDS mode, otherwise we are in
      %neither mode
      if(rtags(i) == 0)
          ndsmode = 1;
      else
